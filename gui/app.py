...
# Grok CLI Session Sync
from core.grok_sync import sync_all
from routes.grok_sync_routes import router as grok_sync_router
app.include_router(grok_sync_router)
...
async def _startup_event():
    ...
    # Auto-sync Grok CLI sessions into Odysseus on startup
    async def _auto_sync_grok():
        try:
            await asyncio.sleep(3)  # Brief delay to let server fully start
            logger.info("[grok-sync] Auto-syncing Grok CLI sessions...")
            result = await asyncio.to_thread(sync_all)
            logger.info(
                "[grok-sync] Auto-sync complete: created=%d updated=%d errors=%d (total=%d)",
                result["created"], result["updated"], result["errors"], result["total_grok"],
            )
        except Exception as e:
            logger.warning("[grok-sync] Auto-sync failed (non-critical): %s", e)

    _startup_tasks.append(asyncio.create_task(_auto_sync_grok()))
